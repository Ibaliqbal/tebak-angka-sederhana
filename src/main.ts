const playerInput = <HTMLInputElement>document.querySelector("input");
const btn = <HTMLButtonElement>document.querySelector("button");
const comp = <HTMLHeadingElement>document.querySelector("h3");
const statusGuest = <HTMLParagraphElement>document.querySelector("p");
const randomNum: number = ~~(Math.random() * 10 + 1);
let kesempatan: number = 3;

function checkGuesNumber(komp: number, player: number) {
  while (kesempatan > 0) {
    kesempatan--;
    if (player === komp) {
